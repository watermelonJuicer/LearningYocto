...
BitBake is fundamentally a generic task execution engine that runs shell and Python tasks efficiently and in parallel while respecting complex inter-task dependencies — OpenEmbedded is just one use-case of that core, which happens to build embedded Linux stacks on top of it.
